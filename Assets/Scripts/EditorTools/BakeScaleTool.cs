...
[EditorTool("Bake Scale", typeof(MeshFilter))]
class BakeScale : EditorTool
{
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                if (GUILayout.Button("Bake Scale"))
                {
                    ...
                    foreach (var obj in targets)
                    {
                        ...
                        {
                            meshFilter = (MeshFilter)obj;
                        }
                    }
                    if (meshFilter == null)
                    {
                        throw new System.Exception("undefined mesh renderer or mesh collider");
                    }
                    ...
                    {
                        throw new System.Exception("mesh filter and mesh collider meshes do not match");
                    }
                    var scaledMesh = MeshUtils.GetScaledMesh(mesh, gameObject.transform.localScale);
                    meshFilter.sharedMesh = scaledMesh;
